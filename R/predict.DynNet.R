...
predict.DynNet <- function(object, newdata, TimeDiscretization=TRUE, MCnr = 10, ui_hat = NULL,...){
  ...
  if(TimeDiscretization){
    ...
    data <- TimeDiscretization(rdata=newdata, subject = subject, 
  ...
  # pred.col(ii+3) = pred_MYtildFull.col(k); //  marginal latent Y prediction E(Ytild) ***
  # pred.col(ii+4) = Res_pred_MYtild.col(k); //  marginal latent Y residual Ytild - E(Ytild)
  ...
  colnames(Marginal_Predict) <- col
  colnames(SS_Predict) <- col.SS
  
  
  return(cbind(Marginal_Predict,SS_Predict))
}
