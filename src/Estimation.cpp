...
mat Sample_Latenti(int K, int nD, arma::mat matrixP, int m_i, arma::vec tau, arma::vec tau_i, arma::mat Ytildi, arma::mat YtildPrimi,
                arma::mat x0i, arma::mat z0i, arma::mat xi, arma::mat zi, arma::colvec alpha_mu0,
                arma::colvec alpha_mu, arma::mat matDw,  arma::mat matDw_u, arma::mat matDu,
                arma::mat matB, arma::mat Sig, arma::mat G_mat_A_0_to_tau_i,
                arma::mat G_mat_prod_A_0_to_tau,  double DeltaT){
  ...
  std::fstream fout("file_check.txt", std::ios::in | std::ios::out | std::ios::app);
  ...
  for( int j =0 ; j < m_i; j++){
    for( int k =j ; k < m_i; k++){
      ...
      matV_i(span(j*nD,j*nD+1),span(k*nD,k*nD+1)) = (phi_0_j_0*z0i)*matDw* (z0i*phi_0_k_0).t() +
        (phi_0_j_0*z0i)*matDw_u*(GrdZi(span(tau_i(k)*nD,(tau_i(k)+1)*nD-1), span(0,q-1))).t() +
        (GrdZi(span(tau_i(j)*nD,(tau_i(j)+1)*nD-1), span(0,q-1)))*matDw_u.t()*(z0i*phi_0_k_0).t() +
        (GrdZi(span(tau_i(j)*nD,(tau_i(j)+1)*nD-1), span(0,q-1)))*matDu*(GrdZi(span(tau_i(k)*nD,(tau_i(k)+1)*nD-1), span(0,q-1))).t()
        ;
      ...
      if(k != j){
        matV_i(span(k*nD,k*nD+1), span(j*nD,j*nD+1)) = matV_i(span(j*nD,j*nD+1),span(k*nD,k*nD+1)).t();
      }
    }
  }
    ...
    for( int k =j ; k < m_i; k++){
      ...
      if( (k_i(j) != 0) & (k_i(k) != 0)){
        ...
        matVY_icheck(span(p_j,(p_j+k_i(j)-1)), span((p_k), (p_k+k_i(k)-1))) += matH_i_t_j*( matrixP*
          (matV_i(span(j*nD,j*nD+1),span(k*nD,k*nD+1)))*matrixP.t())*matH_i_t_k.t();
  ...
  fout << " difference ViY-ViY_check " << ya<<endl;
  
  // Computation choleski
  mat Chol_matV_i = chol(matV_i); //chol.t()*chol = mat

  // Approximation of the likelihood by antithetic MC
  int N_aMC = 50; //Number of pairs of draws for antithetic MC
  double out =0;
  for(int nr=0; nr < N_aMC; nr++){
    // Sampling of Lambda_i
    arma::vec Lambda_nr = mvnorm((nr+1)*666, PNu_cp_i,Chol_matV_i);
    //Computing f(Y|Lambda_i,nr)
    out += f_ord(Lambda_nr, nD, matrixP, tau, tau_i, DeltaT, Ytildi, x0i, alpha_mu0, xi, alpha_mu, G_mat_A_0_to_tau_i);
    
    //Same for pair
    Lambda_nr = mvnorm((nr+1)*666, PNu_cp_i,-Chol_matV_i);
    out += f_ord(Lambda_nr, nD, matrixP, tau, tau_i, DeltaT, Ytildi, x0i, alpha_mu0, xi, alpha_mu, G_mat_A_0_to_tau_i);
